...
export default function ChainlinkDemoPage() {
  ...
  >([])
  const [crossChainLog, setCrossChainLog] = useState<
    Array<{
      id: string
      fromChain: string
      toChain: string
  ...
  useEffect(() => {
    ...
    const savedCrossChainLog = localStorage.getItem("chainlink_ccip_log")
    if (savedCrossChainLog) {
      const log = JSON.parse(savedCrossChainLog).map((e: any) => ({
        ...e,
        timestamp: new Date(e.timestamp),
      }))
      setCrossChainLog(log)
    }
  }, [])
  ...
  const saveCrossChainEvent = (event: any) => {
    const newLog = [event, ...crossChainLog].slice(0, 10)
    setCrossChainLog(newLog)
    localStorage.setItem("chainlink_ccip_log", JSON.stringify(newLog))
  }

  const handleQuickBooksIngest = (project: Project, milestone: Milestone) => {
    const qbEventId = milestone.meta?.qbEventId || "QB-" + Math.random().toString(36).substr(2, 6)
    ...
    addActivity(project.id, `Chainlink Functions: Verified ${milestone.title} (ingested from QuickBooks ${qbEventId})`)
    ...
    saveCrossChainEvent(ccipEvent)

    setProjects(getProjects())

    toast({
      title: "Ingested & Verified via Chainlink",
      description: `${milestone.title} processed from QuickBooks ${qbEventId}`,
    })
  }

  const getQBIngestQueue = () => {
    const queueItems: Array<{ project: Project; milestone: Milestone }> = []
    projects.forEach((project) => {
      project.milestones.forEach((milestone) => {
        if (milestone.meta?.qbSignedOff === true && milestone.status !== "verified") {
          queueItems.push({ project, milestone })
        }
      })
    })
    return queueItems
  ...
  const qbIngestQueue = getQBIngestQueue()
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="space-y-6">
        <Card>
          ...
          <CardContent>
            {qbIngestQueue.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">
                No QB-signed milestones pending verification
              </p>
            ) : (
              <div className="border rounded-lg">
                ...
                <div className="divide-y">
                  {qbIngestQueue.map(({ project, milestone }) => (
                    <div key={`${project.id}-${milestone.id}`} className="grid grid-cols-5 gap-4 p-3 items-center">
                      <div className="font-medium">{project.name}</div>
                      <div>{milestone.title}</div>
                      <div className="font-mono text-sm">
                        {milestone.meta?.qbEventId || "QB-" + Math.random().toString(36).substr(2, 6)}
                      </div>
                      <div>
                      ...
                      <div>
                        <Button size="sm" onClick={() => handleQuickBooksIngest(project, milestone)}>
                          Process via Chainlink
                        </Button>
                      </div>
                    </div>
                  ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Cross-Chain Log</CardTitle>
                <CardDescription>CCIP messages from QuickBooks ingest</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {crossChainLog.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">No cross-chain messages</p>
                  ) : (
                    crossChainLog.map((event) => (
